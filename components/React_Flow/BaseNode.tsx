...
import { CheckCircle, CheckCircle2Icon, Loader2Icon, XCircleIcon } from "lucide-react";
...
>(({ className, status, ...props }, ref) => (
  ...
  >
    {props.children}
    {status === 'error' && (
      <XCircleIcon className="absolute right-0.5 bottom-0.5 size-2 text-red-700/50 stroke-3" />
    )}
    {status === 'success' && (
      <CheckCircle2Icon className="absolute right-0.5 bottom-0.5 size-2 text-green-700/50 stroke-3" />
    )}
    {status === 'loading' && (
      <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 text-blue-700/50 stroke-3 animate-spin" />
    )}
    </div>
));
