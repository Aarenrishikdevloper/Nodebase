  ...
  return (
    <>
      ...
      `}
        ...
        <div className={cn("absolute inset-0 overflow-hidden rounded-sm", classNmae)
        }>
  ...
  switch (status) {
    case "loading":
      switch (variant) {
        ...
        case "border":
          return <BorderLoadingIndicator classNmae={classNmae}>{children}</BorderLoadingIndicator>;
        default:
          return <>{children}</>;
      }
    case "success":
      return (
        <StatusBorder className={cn("border-green-700/50", classNmae)}>{children}</StatusBorder>
      );
    case "error":
      return <StatusBorder className={cn("border-red-700/50",classNmae)}>{children}</StatusBorder>;
    default:
      return <>{children}</>;
  }
};